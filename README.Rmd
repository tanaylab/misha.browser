...
The `expression` field defaults to the vtrack name but can be any valid misha
expression.
...
| `show_direction` | Draw intervals as arrows (triangles) by strand; requires `strand` column (BED col 6 or misha) |
| `direction_field` | Column name for strand (default: `strand`) |

You can also build an intervals panel programmatically from an in-memory data
frame, which is convenient for ad-hoc browsing of computed intervals:
...
Data-frame intervals are runtime-only and are skipped by `browser_save_config()`.
...
Uploaded intervals appear in dropdowns prefixed with `@uploaded:` and can be used as:
- Filter sources for vtracks
- Source for intervals-based vtrack functions (`distance`, `coverage`, `neighbor.count`)
