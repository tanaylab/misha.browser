...
- Sequence-based vtracks (no `src` required) for `kmer.*`, `pwm.*`,
  `masked.*` functions.
- Expression handling uses a single `expression` field.
  If `src`/`func` are present, it wraps the created vtrack during extraction.
  If no `src` or `func` are given, it defines a pure expression vtrack.
...
`colors`, `outline_color`, `label_field`, `show_labels`, `show_direction`
(arrows by strand for directional intervals like CTCF motifs),
`direction_field` (default `strand`), and filtering via
